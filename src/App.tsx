...
const demoCases: Array<{ name: string; detail: string; workflowType: WorkflowType; employeeId: string; shiftId: string; fakeOutcome: FakeOutcome }> = [
  ...
  { name: "Shift confirmation", detail: "Calle Ops · team availability", workflowType: "shift_coordination", employeeId: "emp-lucia", shiftId: "shift-lucia-1", fakeOutcome: "confirmed" },
];
...
const App = () => {
  ...
  const handleReset = async () => {
    ...
    try { const next = await resetState(); setState(next); setPreview(null); setSelectedJobId(null); setView("dashboard"); setHistoryOpen(false); } catch (err) { setError(err instanceof Error ? err.message : "Could not reset"); } finally { setBusy(null); }
  };
    ...
    <div className="app-shell guided-app">
      ...
      <main className={`guided-content ${view === "flow" ? "is-flow" : "is-dashboard"}`}>
        ...
        {view === "dashboard" ? <section className="dashboard-view">
          ...
          <section className="guided-demo panel"><div><p className="eyebrow">GUIDED DEMO</p><h2>Two guided scenarios from three workflows</h2><p>Choose a prepared scenario to see the complete CALL-E approval flow in a deterministic sandbox.</p></div><div className="scenario-grid">{demoCases.map((item, index) => <button className={`scenario-card ${demoCaseIndex === index ? "selected" : ""}`} key={item.name} onClick={() => startScenario(index)}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{item.name}</strong><small>{item.detail}</small></div><b>Start scenario →</b></button>)}</div></section>
          <section className="dashboard-proof"><div><span className="proof-dot" /> CALL-E SANDBOX</div><span>Deterministic outcomes · no call credits used</span><span>{state.jobs.length} saved runs</span></section>
